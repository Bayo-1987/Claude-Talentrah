...
- **Farah panel**: adopted the artboards' full elevated-card treatment (rounded, shadowed, above 760px) — a real departure from Editorial's flat marginalia rule. Below 760px it stays the plain full-width section it always was (no mobile mockup exists to adopt). `src/components/app-shell/farah-panel.tsx`'s own header comment has the exact detail, including why the sticky `top` offset and `(app)/layout.tsx`'s wrapper both changed with it.
