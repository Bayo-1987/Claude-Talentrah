...
- **No consumer of a match score may present more confidence than the screenable-tag denominator supports** — read [docs/match-confidence-invariant.md](docs/match-confidence-invariant.md) before adding anything that reads `match_scores.score`/`.tier` or a fresh `computeMatchScore` result. `describeMatchConfidence` (`src/lib/match-tier.ts`) is the single source of truth every display-shaped consumer (the badge, the weekly digest, the proactive "exceptional match" alert's own display) calls — never build a `${score}% ${tier}` string by hand, which is exactly how both the digest and the proactive alert independently got this wrong before being fixed. A feature whose premise IS a confidence claim (an alert, an autonomous action) needs the stronger eligibility-shaped fix instead — excluding a thin match outright, the way Auto-Apply's own queue gate (0164) and the proactive alert's `isExcellentMatch` both do — not just a capped, honest display of it. `tests/lib/match-confidence-enforcement.test.ts` is the standing check; it fails on a new render site that builds its own percent string instead of calling the shared function, the same way `tests/rls/column-privileges.test.ts` fails on an ungranted column.
...
Verification convention this repo holds itself to, visible throughout its PR history: **check real current state before building; prove a fix by first proving the test catches the bug.** Several milestones caught real defects specifically by re-testing what earlier work had assumed — an RLS policy that had never been run, a retry heuristic that looked like model behaviour, an OAuth name mapping where the intuitive fix would have repaired the wrong provider, and an org-membership policy that read as safe and was not. That last one is also the standing example of a second habit: after fixing a policy, ask what *else* grants the same privilege — the first fix closed one route and, in doing so, opened a second. **send-456 is another instance of this, aimed at a narrower target: a test that exercises the logic AROUND a value, never the value itself.** `tests/support/db-target.test.ts` exhaustively tested `HOSTED_CI_REF`'s behavior without ever asserting what it equalled, so send-455's own repoint could have silently reverted and every test would still pass. The fix is a value-pinning test, proven the same way — reverted locally to confirm it fails, restored to confirm it passes. Anything that carries real identity rather than logic (a project ref, a secret's expected shape, a feature-flag default) needs this same kind of test, not just coverage of the code that reads it.
...
- **6.9 Monetization**: free/uncapped for zero-AI-cost actions (browsing, tracker, algorithmic scoring). One-time free AI trial at signup, not renewable. **Talentrah Credits** for all AI actions beyond that (tailoring, cover letters, bullet rewriting, premium templates, auto-apply beyond free cap). **Passes** (7-day/30-day): auto-renew if paid by card (Paystack/Flutterwave token or Stripe for diaspora), stay prepaid/non-renewing if paid via mobile money wallet. Mentor sessions are NOT a credits action — paid directly, real cash pass-through. Multi-rail payments (mobile money + card) required from day one, not retrofitted. Pricing anchors: credit ≈ ₦150; packs ₦2,500/₦6,000/₦12,500; passes ₦2,000 (7-day) / ₦6,500 (30-day); diaspora subscription $9.99/mo — all researched anchors, **not validated**, need a real pricing test before locking in. **Corrected 2026-09-17: the pack/pass lineup itself has changed, not just the numbers above — `src/lib/billing/catalog.ts` is the living source of truth from here on, don't hand-maintain this line again.** Credit packs (`0089_pricing_catalog_rebase.sql`, 2026-09-03): Starter ₦2,500/20 credits (unchanged), Plus ₦5,000/**50** credits (corrected from 45 by send-417/`0183_plus_pack_credit_count_fix.sql`, 2026-09-19 — 45 was short of what Plus's own marketing copy, "2 tailorings + a cover letter," actually adds up to at current `CREDIT_COSTS`; see `src/lib/billing/catalog.ts`'s `CREDIT_PACK_BUNDLE_CREDITS`), Popular and Power retired (kept, not deleted, for `payment_transactions` referential integrity) — there is no ₦12,500 pack any more. Passes (`0169_pass_price_increase.sql`, 2026-09-17): 7-Day Sprint Pass ₦6,500 (was ₦2,000), 30-Day Pass ₦13,500 (was ₦6,500), 90-Day Pass ₦30,000 (new tier). This is a strategic reversal, not just a price bump: Passes are now deliberately positioned as a premium/heavy-usage product, not the cheaper-than-credits mainstream alternative the original anchors above assumed — see `talentrah-build-prompt.md`'s own superseded-narrative note for the reasoning that changed.
