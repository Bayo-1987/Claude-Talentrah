...
- **A local run now hits CI, not production — but that is a property of this machine, not of the repo.** This line used to say the opposite, and it was true: a committed-nowhere `.env` held production's URL *and* a working `service_role` key, sitting as the fallback layer under `.env.local`. It was deleted on 2026-08-29 after every one of its eleven variables was confirmed to also exist in `.env.local`, so it contributed nothing except a production admin credential one missing file away from being the effective config. **There is no production credential on disk by default any more**, and restoring one re-creates that hazard — so don't, unless you have a specific reason and remove it again afterwards.
