...
- **~47 files still hardcode an ad-hoc bordered box** (`border-[1.5px] border-ink` or similar) instead of using the shared `Card` primitive — down from ~59 as the send-219 sweep started landing this in batches by area (employer forms/panels done; mentorship/talent-directory, admin, and resume-builder still queued) rather than one 70-file PR. Spans admin, employer, mentorship, resume-builder and talent-directory screens. None of them are visually *wrong* (they still use current tokens), just not yet carrying the rounded-corner/shadow treatment `Card` gives everything that does use it, or (where the element is actually a button/input/tag rather than a panel) the matching pill/rounded treatment. Per-file judgment, not a blanket rule: some genuinely stay square on purpose (documented inline where that's the case, e.g. `banner-crop-picker.tsx`'s functional crop viewport).
  - A second, separate pattern the original count missed entirely: small inline tags/status badges using `border border-line px-... text-ink-soft` (a different border weight/token, so it never matched the grep above) — send-219 found and fixed all of these already (job-card.tsx's "sourced externally", tracker-card.tsx's "added manually", the employer masthead's "For employers", both scholarship listing pages' tags, and others).
