...
**Farah's panel is a full elevated card, adopted from the artboards** — `rounded-[18px]`, `background: var(--card)`, the same shared card shadow, `padding: 20px`, `gap: 18px` — above 760px, matching every other Sunbird card. Below 760px it stays the plain full-width section (coral top rule, no card chrome) it always was — neither Sunbird artboard shows a mobile treatment, and a floating card with side margins doesn't translate well edge-to-edge on a phone. The greeting quote gets its own nested `--teal-soft` box (`border-radius: 14px`) at the same breakpoint; the chat input is a full pill (`border-radius: 999px`, `background: var(--bg)`) rather than the bordered square box it used to be. `src/components/app-shell/farah-panel.tsx`'s own header comment has the exact class-by-class detail, including why the sticky `top` offset moved from 68px to 100px (a deliberate 32px gap from the masthead once stuck, which the artboards' static margin doesn't map onto a scrolling sticky element literally) and why the coral top rule no longer appears on the card (not in the artboard, and its original job — marking content inside a now-removed tinted field — no longer applies).
...
~~The Farah panel's marginalia-vs-card treatment~~ — closed: the panel adopted the full elevated-card treatment (§7), including restructuring `(app)/layout.tsx`'s wrapper and re-verifying the Farah e2e specs that depended on its old geometry.
