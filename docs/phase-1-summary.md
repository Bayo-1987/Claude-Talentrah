...
6. **A live posting's description is an unfilled ATS template** — `95318935-744a-4b47-abe5-b0da2877cfa3` (One Acre Fund, "Nigeria Growth and Expansion Senior Associate") contains literal `-` placeholders and a literal `{{Date, Month, Year}}` merge field that made it into the ingested text. Found incidentally during the ESCO coverage validation (`docs/stage8-match-accuracy.md`'s dated 2026-09-15 section) — a source-feed/ingest bug, not a tagging/vocabulary problem, and not fixed here.
7. **A live posting has an empty `description`** — `c1ee6ece-d0e2-4d27-b076-4d65c53b7791` (Apollo Agriculture, "Finance Manager") has zero characters of JD text, so its zero screenable tags are correct given no source text rather than an extraction failure. Also found incidentally during the same ESCO coverage pass; not fixed here.
