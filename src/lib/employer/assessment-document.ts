...
/**
 * The object path an assessment EXERCISE document lives at.
 *
 * `<organization_id>/<job_posting_id>.<ext>` — identical shape to
 * bannerObjectPath (src/lib/employer/banner.ts), for the identical reason:
 * it puts the owning organisation in the first folder segment where 0177's
 * storage policy can read it with `storage.foldername(name)[1]`.
 */
export function exerciseObjectPath(organizationId: string, jobPostingId: string, type: AssessmentDocumentMimeType): string {
  return `${organizationId}/${jobPostingId}.${EXTENSION_FOR[type]}`;
}
...
const EXERCISE_PATH_SHAPE = /^([0-9a-fA-F-]{36})\/([0-9a-fA-F-]{36})\.(pdf|docx|txt)$/;

/**
 * The public URL for a stored assessment EXERCISE document — or null.
 *
 * Same reasoning as bannerPublicUrl: `job_posting_assessments` is written
 * through the employer's own session client (RLS is the real gate), so
 * nothing downstream should trust `exercise_file_path` as a column alone.
 * A URL is only rendered if the path is exactly `<uuid>/<uuid>.<ext>` and
 * its first segment is THIS POSTING'S OWN organisation.
 */
export function assessmentExerciseUrl(args: {
  supabaseUrl: string;
  exerciseFilePath: string | null;
  organizationId: string | null;
}): string | null {
  const { supabaseUrl, exerciseFilePath, organizationId } = args;
  if (!exerciseFilePath || !organizationId) return null;

  const match = EXERCISE_PATH_SHAPE.exec(exerciseFilePath);
  if (!match) return null;
  if (match[1].toLowerCase() !== organizationId.toLowerCase()) return null;

  return `${supabaseUrl.replace(/\/$/, "")}/storage/v1/object/public/${ASSESSMENT_EXERCISE_BUCKET}/${exerciseFilePath}`;
}
