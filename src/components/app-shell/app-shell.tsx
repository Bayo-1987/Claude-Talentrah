 ...
 * marketing masthead + GoogleOneTap (signed-out), plus the Farah panel/tab
 * for a signed-in visitor.
 *
 * EXTRACTED FROM (app)/layout.tsx SO A SECOND layout.tsx CAN RENDER IT.
 * Pure extraction, no behavior change: every branch, every query, every
 * className below is unchanged from the layout file this was pulled out of
 * — this exists so the three public, existence-gated pages that need a
 * SEPARATE loading.tsx ancestry (jobs/[id], scholarships/[id],
 * scholarships/degree/[level] — see that route group's own layout.tsx for
 * why) can render the identical shell without duplicating this logic, while
 * the authenticated product surface keeps its own thin layout.tsx calling
 * this same component. Do not fork this into two copies when adding the
 * second call site — the whole point is one shell, two layouts.
 */
