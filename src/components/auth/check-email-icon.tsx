 ...
 *
 * Path shapes and the coral stroke color are pulled from the validated
 * reference mockup (check-email-redesign.html), not eyeballed: a rectangle
 * plus a chevron/flap line, `--coral` stroke on a `bg-coral-soft` chip — the
 * same accent already used for the eyebrow label directly below it, no new
 * color introduced.
 */
export function CheckEmailIcon() {
  return (
    <div className="flex h-[46px] w-[46px] items-center justify-center bg-coral-soft">
      <svg
        width="22"
        height="22"
        viewBox="0 0 24 24"
