-- 0114: a second way to earn `verified` — CAC business registration.
--
-- ── WHAT THIS ADDS ─────────────────────────────────────────────────────────
--
...
-- `employer_verification` permission (0113). This migration only adds
...
comment on column public.organizations.cac_confirmed_at is
  'When an admin manually confirmed cac_number against the CAC public register. Non-null is what the admin queue (0113''s employer_verification permission) uses to know a submission has already been decided. Service-role write only, like unlisted_at (0107) and removed_at (0056) — it is a trust column, not something the submitting employer may set.';
...
--
-- Fixed here, not filed for later, because shipping 0114's two new trust
...
-- constrained: they carry no trust, and 0114's grant already makes them
