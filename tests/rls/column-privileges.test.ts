...
describe("organizations: a company cannot verify itself at INSERT either (found building 0114)", () => {
  /**
   ...
   * column-scoped, so nothing before 0114 stopped a client naming `verified`
   ...
   * database before 0114's INSERT policy fix existed: `insert into
...
describe("organizations: an employer can submit CAC details but not confirm them (0114)", () => {
  /**
   * The positive control. `cac_number`/`cac_business_name` are exactly the
   * two columns 0114 adds to 0028's `authenticated` UPDATE grant — without
